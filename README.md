# 🎮 Memento Game Studio - Project 1

![Vue.js](https://img.shields.io/badge/vuejs-%2335495e.svg?style=for-the-badge&logo=vuedotjs&logoColor=%234FC08D)
![Vite](https://img.shields.io/badge/vite-%23646CFF.svg?style=for-the-badge&logo=vite&logoColor=white)
![GSAP](https://img.shields.io/badge/gsap-green?style=for-the-badge)
![Express.js](https://img.shields.io/badge/express.js-%23404d59.svg?style=for-the-badge&logo=express&logoColor=%2361DAFB)

Selamat datang di repositori **Memento Game Studio Project 1**. Website ini merupakan platform portofolio interaktif yang menggabungkan desain modern, animasi halus menggunakan GSAP, dan mini-game seru.

## 🚀 Fitur Utama

-   **🏠 Home:** Landing page dengan desain premium dan transisi yang halus.
-   **💼 Work & Service:** Menampilkan proyek-proyek dan layanan studio dengan detail yang komprehensif.
-   **🕹️ Play (Mini-Game):** Menampilkan game **Cat Tank Basketball**, sebuah game interaktif yang dibangun menggunakan Vue dan animasi GSAP.
-   **📱 Responsive Design:** Dioptimalkan untuk berbagai ukuran layar (Mobile, Tablet, Desktop).
-   **✨ Smooth Animations:** Menggunakan GSAP untuk pengalaman pengguna yang lebih hidup dan premium.

## 🛠️ Tech Stack

-   **Frontend:** [Vue.js 3](https://vuejs.org/) (Composition API)
-   **Build Tool:** [Vite](https://vitejs.dev/)
-   **Styling:** Vanilla CSS (Custom UI/UX Design)
-   **Animation:** [GSAP (GreenSock Animation Platform)](https://greensock.com/gsap/)
-   **Routing:** Vue Router 4
-   **Backend:** Express.js (untuk server pendukung)

## 📦 Struktur Folder

```text
Project1/
├── dist/               # Build produksi
├── server/             # Backend server (Express)
├── src/                # Source code frontend
│   ├── components/     # Komponen Vue (CatTankBasketball, SocialIcons, dll)
│   ├── views/          # Halaman utama (Home, Play, Work, dll)
│   ├── router/         # Konfigurasi Vue Router
│   └── assets/         # Gambar dan media
├── index.html          # Entry point HTML
├── main.js             # Entry point Frontend
└── vite.config.js      # Konfigurasi Vite
```

## ⚙️ Instalasi & Penggunaan

### 1. Clone Repositori
```bash
git clone https://github.com/azzam380/memento_p1.git
cd Project1
```

### 2. Install Dependensi
```bash
npm install
```

### 3. Jalankan Mode Pengembangan
```bash
npm run dev
```
Buka `http://localhost:5173` di browser Anda.

### 4. Build untuk Produksi
```bash
npm run build
```

### 5. Jalankan Server Produksi
```bash
npm run server
```

## 👨‍💻 Kontributor
- **Muhammad Azzam and Benneth** - *Lead Developer & Designer*

---
Dibuat dengan ❤️ oleh [Memento Game Studio](https://github.com/azzam380).
